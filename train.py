...
class ResNetFPNModel(DetectionModel):
    ...
    def inputs(self):
        ...
        if cfg.MODE_MASK:
            ret.append(
                tf.placeholder((None, None, None), tf.uint8, 'gt_masks')
            )   # NR_GT x height x width
        return ret
...
def do_evaluate(pred_config, output_file):
    ...
    for dataset in cfg.DATA.VAL:
        ...
        if (len(dataset.shape) == 2):
            ...
            dataset = np.repeat(dataset, 3, axis=2)
        dataflows = [
...
def do_predict(pred_func, input_file):
    ...
    logger.info("Inference output for {} written to outputimg.tif".format(input_file))
    return results
    #tpviz.interactive_imshow(viz)
...
class BboxPredictor:
    def __init__(self, path):
        ...
        self.pred_func = OfflinePredictor(PredictConfig(
            model=self.model,
    ...
    def predict_bbox(self, image):
        ...
        results = detect_one_image(image, self.pred_func)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logdir', help='log directory', default='/scratch/ganswindt/COCO/Yeasts_mating/train_log_1906')
    ...
    print('Used Log Directory: ', args.logdir)
