...
def rpn_losses(anchor_labels, anchor_boxes, label_logits, box_logits):
    ...
    delta = 1.0 / 9
    class_weights = tf.constant([0, 0.3, 0.4, 0.3])
    weights = tf.gather(tf.cast(class_weights, tf.int32), tf.cast(pos_anchor_boxes, tf.int32))

    box_loss = tf.losses.huber_loss(
        pos_anchor_boxes, pos_box_logits, weights, delta=delta,
